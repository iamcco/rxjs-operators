...
type TweenName = keyof typeof tweenFunctions
...
export function Tween(
  ...
  if (!tweenFunctions[name]) {
    return throwError(`No such tween function name: ${name}!`)
  }
  return Observable.create((observer: Observer<number>) => {
    let isStop = false
    const tw = tweenFunctions[name]
    const startTime = Date.now()
    const frame = () => {
      const ct = Date.now() - startTime
      observer.next(
        tw(ct >= duration ? duration : ct, from, to, duration, s)
      )
      if (!isStop && ct < duration) {
